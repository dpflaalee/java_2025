...
public class BoardService {
	...
	public Board find(Long id) {  //상세보기
		Board board = boardMapper.findById(id);  //해당번호 찾아서
		if(board != null) {  //조회수 올리고 return
	...
	public int insert (MultipartFile file, Board board) throws IllegalAccessException { //글쓰기
		//파일 업로드, 작성자 정보조회, bhit=0, ip주소설정
		String username = board.getMember() != null ? board.getMember().getUsername() : null;
		if(username != null) {
			Member member = memberMapper.findByUsername(username);
			if(member != null) {board.setMember(member); } 
			else {throw new IllegalAccessException("유효하지 않은 사용자입니다.");}
		}
	...
	public int update(MultipartFile file, Board board) {
		...
		if(file != null && file.getOriginalFilename().length()>0) {
			try { save = upload.fileUpload(file);}catch (IOException e) {e.printStackTrace();}
		}
		board.setBfile(save);
		return boardMapper.updateByIdAndBpass(board);
	};
	
	public int delete (Board board) {return boardMapper.deleteByIdAndBpass(board);}; //삭제
